  ...
  public:
    ...
    uint64_t sim_RAW_hits, roi_RAW_hits, //Loads hitting in Store queue
	     ...
	     sim_load_sent, roi_load_sent, //Loads sent to L1D cache
             ...
             sim_partial_RAW_hits, roi_partial_RAW_hits;    // Loads having only partial overlapping with stores in store queue

    uint64_t stall_cycles_not_scheduled = 0;
    uint64_t stall_cycles_scheduled_not_executed = 0;
    uint64_t stall_cycles_waiting_mem_ops = 0;
    uint64_t stall_cycles_inflight_no_mem_ops = 0;
    uint64_t memory_instrs_during_warmup = 0;
    uint64_t non_memory_instrs_during_warmup = 0;
    uint64_t mem_loads_which_are_forwarded_during_warmup = 0;
    uint64_t num_mem_loads_during_warmup = 0;
    uint64_t num_mem_stores_during_warmup = 0;
    // TLBs and caches
    ...
    void add_load_queue(uint32_t rob_index, uint32_t data_index, uint64_t address, uint32_t block_offset, uint32_t data_size, uint32_t lq_data_index, unsigned char data_value[64]),
         add_store_queue(uint32_t rob_index, uint32_t data_index, uint64_t address, uint32_t block_offset, uint32_t data_size, uint32_t sq_data_index, unsigned char data_value[64]),
