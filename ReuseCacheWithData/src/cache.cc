...
#ifdef SANITY_CHECK
                        if (RQ.entry[index].data == 0)          // data_Value zero check should be added. as of now left data field also as is since tlb caches are using that to get physical address or check with zero are many
                            assert(0);
#endif
                    }
